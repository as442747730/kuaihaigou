<template>
  <div class="m-invoice-edit">
    ...
    <div class="m-invoice-edit-form">
      <div class="m-invoice-edit-form-item">
      ...
      <transition name="nav-fade">
        <div class="m-invoice-edit-form-item" v-if="headType === 2">
    ...
    <transition name="nav-fade">
      <div class="m-invoice-edit-form" v-if="invoiceType === 2">
          <div class="m-invoice-edit-form-item" >
        ...
        <div class="m-invoice-edit-form-item small"></div>
      </div>
    </transition>

    <div class="m-invoice-edit-tip">
      <p>发票须知</p>
    ...
    <div :class="['m-invoice-edit-btn', saveDisabled ? 'disabled' : 'active-status']" @click="createInvoice">保存</div>
...
export default {
  ...
  async asyncData (req) {
    return api.serverGet('/api/invoice/get/' + req.params.edit).then((res) => {
      ...
      if (res.code === 200) {
        ...
        return {
          invoiceId: req.params.edit,
          headType: res.data.headType,
          invoiceType: res.data.invoiceType,
          head: res.data.head,
          taxIdentify: res.data.taxIdentify,
          email: res.data.email,
          contentType: res.data.contentType,
          contentText: res.data.contentType === 1 ? '酒水/饮料' : '商品明细'
        }
      }
    })
  },
  ...
  methods: {
    ...
    async createInvoice () {
      ...
      const { code, data } = await api.clientPostJson('/api/invoice/update', {
        id: this.invoiceId,
